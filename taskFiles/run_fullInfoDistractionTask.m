%% Task script to run Distraction task with full-information distractions
...
if isequal(run,1)
    ...
    % experiment settings
    data.rate.blockname = {'HEALTH' 'TASTE' 'FAMILIARITY' 'FREQUENCY' 'WANTING'};
    data.rate.blockdistript = {'HEALTHY you think each food is', ...
        'TASTY each food item is to you', ...
        'FAMILIAR each food item is to you', ...
        'FREQUENTLY you eat each food item', ...
    ...
    data.rate.ratingsorder = [randperm(4) 5]; % wanting LAST, other two random
    ...
    data.rate.guide{3} = {'not familiar n\at all', 'a little familiar', 'somewhat familiar', 'familiar', 'very familiar'};
    data.rate.guide{4} = {'have never \neaten it', 'rarely eat it', 'sometimes eat it', 'often eat it', 'eat it all\nthe time'};
    data.rate.guide{5} = {'strongly don''t want', 'don''t want', 'neutral', 'want', 'strongly want'};
    ...
    data.rate.guideTwo{3} = {'very familiar','familiar','somewhat familiar','a little familiar','not familiar \nat all'};
    data.rate.guideTwo{4} = {'eat it all\nthe time','often eat it','sometimes eat it','rarely eat it','have never \neaten it'};
    data.rate.guideTwo{5} = {'strongly\ndon''t want', 'don''t want', 'neutral', 'want', 'strongly\nwant'};
    data.rate.choicevalence_guide = repmat(-2:2,5,1);
    ...
    inst{1} = ['In this part of the study, you will participate in two tasks.\n\n\n'...
        ...
        'of foods on five attributes, one at a time.\n\n'...
        'Press the right arrow to continue.'...
        ];
    inst{length(inst)+1} = ['You''ll rate foods on their healthiness, how tasty they are to you, how familiar you are with them, how frequently you eat them, and how much you want to eat them.\n\n'...
    ...
    tic;
    count=1;
    for nB = data.rate.ratingsorder(1:5) % collect health and taste in the pre-determined order
    ...
    data.binary.leftFoodLoc = [data.width*relative_position-food_width data.height*.5-food_height ...
            data.width*relative_position+food_width data.height*.5+food_height];
    data.binary.rightFoodLoc = [data.width*(1-relative_position)-food_width data.height*.5-food_height ...
            data.width*(1-relative_position)+food_width data.height*.5+food_height];
    data.binary.leftFoodLocAOI = [data.width*relative_position-(food_width+aoi_border) data.height*.5-(food_height+aoi_border) ...
            data.width*relative_position+(food_width+aoi_border) data.height*.5+(food_height+aoi_border)];
    data.binary.rightFoodLocAOI = [data.width*(1-relative_position)-(food_width+aoi_border) data.height*.5-(food_height+aoi_border) ...
            data.width*(1-relative_position)+(food_width+aoi_border) data.height*.5+(food_height+aoi_border)];
    ...
    data.binary.left_box = [data.width*relative_position-box_width data.height*.5-box_height ...
        data.width*relative_position+box_width data.height*.5+box_height];
    data.binary.right_box = [data.width*(1-relative_position)-box_width data.height*.5-box_height ...
        data.width*(1-relative_position)+box_width data.height*.5+box_height];
    ...
    data.binary.distractor.onset = NaN(data.binary.nTrials,1);
    data.binary.distractor.length = NaN(data.binary.nTrials,1);
    save(dataFName,'data');
    sca
...
elseif isequal(run,2)
    ...
    for trial = 1:data.binary.nTrials 
        ...
        if ismember(trial,data.distractor_trials) 
            % Setup boolean to check for second fixation
            first_fix_made = false;
            first_fix_loc = 0;
            disp('Distraction trial!')
            ...
            data.binary.distractor.length(trial) = (randi([400 1000],1,1))/1000;
            data.binary.distractor.onset(trial) =  (randi([83 183],1,1))/1000;
            DistractorTimeFrames = round(data.binary.distractor.length(trial) / ifi);
            ...
            if choice_made == false;            
                ...
                j = 0;
                gazeData.left_distInit{trial}              = [];
                gazeData.right_distInit{trial}             = [];
                gazeData.timestamp_distInit{trial}         = [];
                gazeData.sz_distInit{trial}                = [];            
                ...
                while 1
                    ...
                    [lefteye_distInit, righteye_distInit, timestamp_distInit, trigSignal_distInit] = tetio_readGazeData;
                        gazeData.left_distInit{trial}              = [gazeData.left_distInit{trial};lefteye_distInit];
                        gazeData.right_distInit{trial}             = [gazeData.right_distInit{trial};righteye_distInit];
                        gazeData.timestamp_distInit{trial}         = [gazeData.timestamp_distInit{trial};timestamp_distInit];
                        gazeData.sz_distInit{trial}                = [gazeData.sz_distInit{trial};trigSignal_distInit];
                    ...
                    % Determines the location of the first fixation, and
                    % then sets parameters to calculate the duration of the
                    % second fixatio
                    if first_fix_made == false && ~isempty(eyes(((eyes(:,2) > data.binary.leftFoodLocAOI(1) & eyes(:,2) < data.binary.leftFoodLocAOI(3)) & (eyes(:,3) > data.binary.leftFoodLocAOI(2) & eyes(:,3) < data.binary.leftFoodLocAOI(4)))))
                        first_fix_made = true;
                        first_fix_loc = 2;
                    elseif first_fix_made == false && ~isempty(eyes(((eyes(:,2) > data.binary.rightFoodLocAOI(1) & eyes(:,2) < data.binary.rightFoodLocAOI(3)) & (eyes(:,3) > data.binary.rightFoodLocAOI(2) & eyes(:,3) < data.binary.rightFoodLocAOI(4)))))
                        first_fix_made = true;
                        first_fix_loc = 3;
                    end
                    if first_fix_made == true && first_fix_loc == 2;
                        eyes_temp = eyes(((eyes(:,2) > data.binary.rightFoodLocAOI(1) & eyes(:,2) < data.binary.rightFoodLocAOI(3)) & (eyes(:,3) > data.binary.rightFoodLocAOI(2) & eyes(:,3) < data.binary.rightFoodLocAOI(4))));
                      elseif first_fix_made == true && first_fix_loc == 3;
                        eyes_temp = eyes(((eyes(:,2) > data.binary.leftFoodLocAOI(1) & eyes(:,2) < data.binary.leftFoodLocAOI(3)) & (eyes(:,3) > data.binary.leftFoodLocAOI(2) & eyes(:,3) < data.binary.leftFoodLocAOI(4))));
                    else
                    end
                    ...
                    % Checks to see if enough fixations in AOI
                    [keyIsDown, ~, keyCode] = KbCheck;
                    ...
                    elseif first_fix_made == true
                        if size(eyes_temp,1) >= round(data.binary.distractor.onset(trial)/ifi) %| keyIsDown && any(keyCode(data.binary.resp_key_codes)) && choice_made == false
                            first_fix_made = false;
                            first_fix_loc = 0;
                            break;
                        end;
                    ...
                    data.binary.timeStamp.SecondFixationCompleted(trial) = GetSecs;   
                end
            ...
            RemainingTimeFrames = round(((4-(data.binary.timeStamp.SecondFixationCompleted(trial) - data.binary.timeStamp.stimOn(trial)))/ ifi));
        ...
        % No distraction condition.
            ...
            data.binary.distractor.type(trial) = 0;
                for frame = 1: round(Trial_Length/ifi) - 1
                    ...
                    Screen('TextSize', exp_screen, txt_size.fixCross);
                    Screen(exp_screen, 'FillRect', bg_color);
                    % get images  and draw boxes around foods
                    Screen('DrawTexture', exp_screen, choiceImg{1}, [], data.binary.leftFoodLoc);
                    Screen('DrawTexture', exp_screen, choiceImg{2}, [], data.binary.rightFoodLoc);    
                    Screen('FrameRect', exp_screen, txt_color, data.binary.left_box, box_left_thickness);
                    Screen('FrameRect', exp_screen, txt_color, data.binary.right_box, box_right_thickness);              
   ...
   gazeData.left{trial}              = lefteye;
   gazeData.right{trial}             = righteye;
   gazeData.timestamp{trial}         = timestamp;
   gazeData.sz{trial}                = eyesize;
    toc
    ...
    %% Require participants to fixate back at distractor. 
    eyes = [0 0 0]; % this always need to be initiated or else error
    eyes2 = {'TimeStamp','EyeX','EyeY','RawGazeData'};
    j = 0;                
    gazeData.left_fixation{trial}              = [];
    gazeData.right_fixation{trial}             = [];
    gazeData.timestamp_fixation{trial}         = [];
    gazeData.sz_fixation{trial}                = [];                        
    while 1
        pause(1/data.FrameRate); % measurement every *almost* 16 millisec - probably closer to 20 millisec, dependeing on computing power
        [lefteye_fixation, righteye_fixation, timestamp_fixation, trigSignal_fixation] = tetio_readGazeData;
        gazeData.left_fixation{trial}              = [gazeData.left_fixation{trial};lefteye_fixation];
        gazeData.right_fixation{trial}             = [gazeData.right_fixation{trial};righteye_fixation];
        gazeData.timestamp_fixation{trial}         = [gazeData.timestamp_fixation{trial};timestamp_fixation];
        gazeData.sz_fixation{trial}                = [gazeData.sz_fixation{trial};trigSignal_fixation];
                    
        % this makes sure that at least one eye is being picked up
        % NECESSARY or else will get error with ParseGazeData
        if isempty(lefteye_fixation)
            continue;
        end
                
        numGazeData = size(lefteye_fixation, 2);
        leftEyeAll = vertcat(leftEyeAll, lefteye_fixation(:, 1:numGazeData));
        rightEyeAll = vertcat(rightEyeAll, righteye_fixation(:, 1:numGazeData));
        timeStampAll = vertcat(timeStampAll, timestamp_fixation(:,1));
        trialNum = vertcat(trialNum, repmat(trial,size(timestamp_fixation(:,1),1),1));

        GazeData = ParseGazeData(lefteye_fixation(end,:),righteye_fixation(end,:)); % Parse last gaze data.
        ...
        if validLeftEyePos && validRightEyePos
            ...
            eyes = [eyes;[GetSecs,mean([l_x,r_x]),mean([l_y,r_y])]];
            eyes2 = [eyes2;[num2cell([GetSecs,mean([l_x,r_x]),mean([l_y,r_y])]),{GazeData}]];
        end
                    
        eyes_temp_fix = eyes((eyes(:,2) > data.binary.distractorLoc(1) & eyes(:,2) < data.binary.distractorLoc(3)) & (eyes(:,3) > data.binary.distractorLoc(2) & eyes(:,3) < data.binary.distractorLoc(4)));
        if size(eyes_temp_fix,1) >= 12 %| keyIsDown && any(keyCode(data.binary.resp_key_codes)) && choice_made == false 
            break;
        end
    end     
    ...
    if trial == round(data.binary.nTrials/2);
        Screen('TextSize', exp_screen, txt_size.fixCross);
        Screen(exp_screen, 'FillRect', bg_color);
        DrawFormattedText(exp_screen,'You are at the halfway point. \n\n\n This is a 30 second break. The task will continue afterwards.','center','center',txt_color,wrapat,[],[],vSpacing);     
        Screen(exp_screen,'Flip');
        WaitSecs(30);
        for i = 1:5
            Screen(exp_screen, 'FillRect', bg_color);
            DrawFormattedText(exp_screen,['Task will start in ' num2str(6-i) ' seconds.\n\n\n'],...
                'center', 'center', [255 0 0],wrapat,[],[],vSpacing);
            Screen(exp_screen,'Flip');        
            WaitSecs(1);
        end
