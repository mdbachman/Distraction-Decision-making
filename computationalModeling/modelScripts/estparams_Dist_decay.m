...
savefile = fullfile(dir,'results',...
    ['output_', num2str(s) cond num2str(grp) 'Data_decay.mat']);
...
for i = 1:size(possparams,1)
    %i/size(possparams,1)
    gridchisq_all(i) = fitFcn_Dist_decay(vals, realchoices, realrts, distonset, distoffset, drate, ndt, boundary, nsims, ...
        possparams(i,:),preDistLength,distLength);
end
gridtime=toc;
...
[estparams_all, chisq_all, exitflag_all, output_all, population_all, scores_all] = ...
    ga(@(params)(fitFcn_Dist_decay(vals, realchoices, realrts, distonset, distoffset, drate,ndt, boundary, nsims, params,preDistLength,distLength)), ...
