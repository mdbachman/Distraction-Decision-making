function chisq = fitFcn_addm_noDist_firstFixnoRV(vals, realchoices, realrts, nsims, gaze, params)
%% Fits the attention drift-diffusion model
% Based on Equation 3 in Methods.
...
for i = 1:size(possdiffs,1)
    ...
    for j = 1:nsims
        ...
        for idx = 1:length(thispat)
            ...
            % This is the key modification here.
            if idx == 1
                fixdrate =  drate(i,fItem); % for first fixation, only take the value of the fixated item.
            else
    ...
    if Ntop>0
        ...
        RTbins_top = [0 quantile(realrts(realchoices==1 & valind),  topquants) Inf];
        
        % for simulated data, get probability
        if any(simchoices == 1)
            % bin simulated RTs using real top choice bins
            [~, ~, simrtbin] = histcounts(simrts(simchoices ==1), RTbins_top);
            % get p(this choice & RT bin)
            for j = 1:length(RTbins_top)-1
                pi(i, j, 1) = sum(simrtbin == j) /...         
        ...
        [~, ~, realrtbin_top] = histcounts(realrts(realchoices==1 & valind), RTbins_top);
        for j = 1:length(RTbins_top)-1
            % get p(this choice & RT bin)
            p(i,j,1) = max([eps, ...
                sum(realrtbin_top==j)./ ...
    ...
    if Nbot>0
        ...
        RTbins_bot = [0 quantile(realrts(realchoices== -1 & valind), botquants) Inf];

        
        if any(simchoices==-1)
            [~, ~, simrtbin] = histcounts(simrts(simchoices==-1), RTbins_bot);
            for j = 1:length(RTbins_bot)-1
                pi(i, j, 2) = sum(simrtbin == j) / ...
                    sum(simchoices==-1);% predicted proportion
            end
        end
        pi(i,pi(i,:,2)==0,2)=eps; % avoiding infinity

        [~, ~, realrtbin_bot] = histcounts(realrts(realchoices==-1), RTbins_bot);        
        for j = 1:length(RTbins_bot)-1
            p(i,j, 2) = max([eps, ...
                sum(realrtbin_bot==j)./ ...
                sum(realchoices == -1 & valind)]);
