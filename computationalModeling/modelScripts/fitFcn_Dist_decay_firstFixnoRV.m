function chisq = fitFcn_Dist_decay_firstFixnoRV(vals, realchoices, realrts, distonset, distoffset, drate, ndt, boundary, nsims, params, gaze,preDistLength,DistLength,firstFixDur)
%% Fits DDM parameters
...
for i = 1:size(possdiffs,1)
    ...
    for thisSim = 1:nsims
        ...
        % Distractor period, with decay function.
        for zz = kk+1:kk+this_distLength
            drift(thisSim,zz) = decayParam* drift(thisSim,kk) + normrnd(0, noise);
        end
        drift(thisSim,zz+1:end) = drift(thisSim,zz) + cumsum([ (repmat((drate(i,1)-drate(i,2)), 1, maxrt-zz))  + normrnd(0, noise,1, maxrt - zz)], 2);
    end
