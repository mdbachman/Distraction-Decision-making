...
% This particular one is for Equation 2 in Methods.
...
try 
    ...
    if isrecov
        ...
        savefile = fullfile(dir,'results_addm',...
            ['recov_addm_', num2str(s) 'Data.mat']);
    else
        fprintf('s %d, %s exp %d\n',s, cond, grp)
        savefile = fullfile(dir,'results_addm',...
            ['addm_s', num2str(s) '_' cond '_set' num2str(grp) '_Data_v2.mat']);
    end
    ...
    if ~exist('gridsorted_all','var')
        ...
        for i = 1:size(possparams,1)
            gridchisq_all(i) = fitFcn_addm_noDist(vals, realchoices, realrts, nsims, ...
                gaze, possparams(i,:));
        end
    ...
    %% GA fit
        ...
        [estparams_all, chisq_all, exitflag_all, output_all, population_all, scores_all] = ...
            ga(@(params)(fitFcn_addm_noDist(vals, realchoices, realrts, nsims, gaze, params)), ...
