function chisq = fitFcn_Dist_2ndSampleDistDur2(vals, realchoices, realrts, distonset, distoffset, drate, ndt, boundary, nsims, params, gaze,predistLength,distLength,firstFixDur)
%% Fits DDM parameters
% This particular one is for accumulation models with NO modified first
% fixation.
...
for i = 1:size(possdiffs,1)
    ...
    % simulate process
        drift(thisSim,:) = cumsum([zeros(1, ndt) ... % for speed omit bias=0 here
            (repmat(mu*drate(i), 1, this_distLength)  + normrnd(0, noise, 1, this_distLength)) ...
            (repmat(drate(i), 1, maxrt-(ndt+this_distLength)) + normrnd(0, noise, 1, maxrt-(ndt+this_distLength))) ... 
            ], 2);
    end
